						...
						<div class="panel panel-default">
							<div class="panel-heading" style="text-align: center;" id="exTab2">	
								<ul class="nav nav-pills nav-justified">
									<li class="active">
									<a  id="1a" href="#1b" data-toggle="tab">News Slider</a>
									</li>
									<li><a id="2a" href="#2b" data-toggle="tab">News Ticker</a>
									</li>
									<?php
							...
							<div class="panel-body tab-content ">
								<div class="tab-pane active" id="1b">
									<?php
										if($_SESSION['name'][0]=='Administrator') {
											...
											WHERE `status` = 'Approved' AND posttype = 0 AND showbulletin <> 2 AND posts.posterid = :posterid ORDER BY datetime DESC");	
											...
											LEFT JOIN administrators
											ON administrators.id = posts.posterid
											WHERE `status` = 'Approved'  AND showbulletin <> 2 AND posttype = 0 ORDER BY datetime DESC");	
											$stmt->execute();											
										}
											//approved already
											foreach(($stmt->fetchAll()) as $row) { 
												...
												'<a data-toggle="collapse" href="#collapsePanel' . $row['id'] . '"><strong>' . $row['poster'] . '</strong></a> @ <i>' . relativeTime($time) . '</i>';
													if (($row['filetype'] == 'gif')||($row['filetype'] == 'jpg')||($row['filetype'] == 'png')||($row['filetype'] == 'webp')||($row['filetype'] == 'jpeg')) {
														echo '</div><div id="collapsePanel'.$row['id'].'" class="panel-collapse"><div class="panel-body"><div class="col-lg-2">' .
														...
														echo '</div><div id="collapsePanel'.$row['id'].'" class="panel-collapse"><div class="panel-body"><div class="col-lg-12"><strong>' . $row['posttitle'] . '</strong><hr/>' . $row['post'] . '</div>';
													} else {
														echo '</div><div id="collapsePanel'.$row['id'].'" class="panel-collapse"><div class="panel-body"><div class="col-lg-12"><strong>' . $row['posttitle'] . '</strong><hr/>' . $row['post'] . '<hr/>
								...
								<div class="tab-pane" id="2b">
									<?php
										if($_SESSION['name'][0]=='Administrator') {
											...
											LEFT JOIN administrators
											ON administrators.id = posts.posterid
											WHERE `status` = 'Approved' AND posttype = 1 AND posts.posterid = :posterid ORDER BY datetime DESC");	
											$stmt->bindParam(':posterid', $_SESSION['name'][2]);
											$stmt->execute();
										} else {
											$stmt = $conn->prepare("SELECT posts.*, administrators.name as poster from `posts` 
											LEFT JOIN administrators
											ON administrators.id = posts.posterid
											WHERE `status` = 'Approved' AND posttype = 1 ORDER BY datetime DESC");	
											$stmt->execute();											
										}
										//approved already
											foreach(($stmt->fetchAll()) as $row) { 
									...
									<?php
										...
										$stmt->execute();
											...
											foreach(($stmt->fetchAll()) as $row) { 
												...
												echo '<div class="panel panel-default"><div class="panel-heading">' . 
												'<a data-toggle="collapse" href="#collapsePanel' . $row['id'] . '"><strong>' . $row['poster'] . '</strong></a> @ <i>' . relativeTime($time) . '</i>';
													if (($row['filetype'] == 'gif')||($row['filetype'] == 'jpg')||($row['filetype'] == 'jpeg')||($row['filetype'] == 'png')||($row['filetype'] == 'webp')) {
												...
												if ($row['showbulletin']=='1') { 
													...
													echo '<option value="1">Show</option> 
													<option value="0">Hide</option>
													<option value="2" selected>Dismiss</option>';
												}
												echo '</select>';
												echo '</div></div>';
											}*/
