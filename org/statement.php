...
<body>
										...
										<table class="table table-bordered">
											...
											<?php
												foreach(($stmt->fetchAll()) as $row) { 
													...
													echo '<td id="studnum">' . $row['studnum'] . '</td>';
												}
											?>
											</tr>
											</tbody>
										...
										<table id="statement" class="table table-bordered">
											...
											<?php
												...
												foreach(($stmt->fetchAll()) as $row) { 
													...
													echo '<td contentEditable style="border: 2px solid #c3aeba; ">' . $row['amountpaid'] . '</td></tr>';
												}
												$conn=null;
											?>
											</tbody>
	...
	<script>
		$("#saveStatement").click(function(){
			var studentid = $('#studnum').text();
			var statement = $('#statement').tableToJSON({
				ignoreColumns: [1, 2]
			});
			var $orgid = $('#userid').text();
			//alert(studentid);
			//alert(JSON.stringify(statement));
			$.ajax({
				type: "POST",
				url: "/php/savePayments.php",
				data: {jsonstatement: JSON.stringify(statement), studnum: studentid, orgid: $orgid},
				cache: false,
